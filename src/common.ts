...
export const runAsync = (fn: () => unknown, options?: { done?: (result: unknown) => void, error?: (e: Error) => void }) => {
    (async () => fn())()
        .then(result => options?.done?.(result))
...
export const cleanMapAsync = async <K extends string = string, V = any>(map: { [key in K]: V }, cb: (key: K, val: V) => void | Promise<void>) => {
    for (let key of Object.keys(map) as K[]) {
        const val = map[key];
        delete map[key];
        await cb(key, val);
    }
